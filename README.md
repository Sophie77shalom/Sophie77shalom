 # Hi, I'm Sophy 👋
🎀 Bsc. Computer Science @ Kirinyaga University
📍 Nairobi, Kenya

I'm passionate about turning data into actionable insights and building AI solutions that solve real-world problems.
I enjoy working with data, developing machine learning models, and creating intelligent systems that support better decision-making.
Looking for remote AI & Machine Learning Engineering / Data Analytics opportunities.

## 💻 What I Work With

- Python (Pandas, NumPy, Scikit-learn)
- Machine Learning & Time Series
- Data Analytics
- SQL & ETL
- Power BI & Excel
- FastAPI & Model Deployment

---

## 📊 Projects

- 🌱 **AgriCheck** – AI-powered crop disease detection system using computer vision and multimodal AI
- 🎬 **K-Drama Recommendation System** – Personalized recommendation engine based on user preferences and similarity modeling
- 📈 **Stock Price Prediction** – Time series forecasting using historical market data and predictive analytics
- 🏞️ **Landmark Classification & Tagging for Social Media** – Computer vision solution for landmark recognition and automated tagging
- 📉 **Investor Risk Analytics Dashboard** – AI-driven risk scoring, explainability, and market insights for investors

---

## 🎓 Research & Interests

- Machine Learning for Financial Analytics
- Explainable AI (SHAP & Model Interpretability)
- Recommendation Systems
- AI for Agriculture and Social Impact
- Data-Driven Product Development

---

## ✨ Currently

- Building AI and machine learning projects
- Exploring Generative AI and AI Agents
- Contributing to open-source and portfolio projects
- Preparing for AI/ML Engineer and Data Analytics opportunities

---

## 🌱 Goals

- Build impactful AI solutions for real-world problems
- Grow as an AI/ML Engineer and Data Scientist
- Contribute to innovative AI products
- Continuously learn and stay updated with emerging technologies

---

## Github stats 📈

[![Sophie’s github stats](https://github-readme-stats.vercel.app/api?username=Sophie77shalom)](https://github.com/Sophie77shalom)

[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Sophie77shalom&layout=compact)](https://github.com/Sophie77shalom)

 
## Contact Me :telephone_receiver:
📫 Let's connect and collaborate on AI, Machine Learning, Data Science, and Analytics projects.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/sophie-jeptoo-0435bb240/) [![TWITTER](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/Sophieshalom12) [![INSTAGRAM](https://img.shields.io/badge/Instagram-E4405F?style=for-the-badge&logo=instagram&logoColor=white)](https://www.instagram.com/shalom3527/)

